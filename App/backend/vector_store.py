...
#     print(f"Added{len(text_chunks)} chunks to the FAISS index.")
...
# def search_similar_chunks(question,k=10):
#     # Handle empty dataset
#     if len(document_chunks)==0:
#         return["No documnet uploaded yet"]
...
#     # Search FAISS
#     distances,indices=index.search(np.array(question_embedding).astype("float32"),k=k)
#     retrieved_chunks=[]
#     for i in indices[0]:
#         # FAISS can return -1
#         if i!=-1:
...
#     for i in indices[0]:
#         if i != -1:
#             retrieved_chunks.append(document_chunks[i]) # now dict with text + file_name
#     return retrieved_chunks
